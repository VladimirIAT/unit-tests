...
public class Calculator {
    ...
    public static double calculatingDiscount(double purchaseAmount, int discountAmount) {
        if (purchaseAmount <= 0) {
            throw new ArithmeticException("It is not possible. purchaseAmount must be more than 0");
        }
        if ((discountAmount >= 100) || (discountAmount < 0)) { //
            throw new ArithmeticException("It is not possible. discountAmount must be more than 0 and less 100");
        }
        double totalAmount = purchaseAmount - (purchaseAmount * discountAmount / 100);
        return totalAmount;
    }
}